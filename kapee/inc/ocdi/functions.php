...
function kp_ocdi_import_files() {
    $dd_path = trailingslashit( get_template_directory() ) . 'demo-content/';
    return apply_filters( 'kp_ocdi_files_args', array(
        array(
            'import_file_name'             => 'kapee',
            'categories'                   => array( 'kapee' ),
            ...
            'import_notice'                => esc_html__( 'Import process may take 10-15 minutes. If you facing any issues please contact our support.', 'kapee' ),
        ),
    ) );
}

function kp_ocdi_after_import_setup( $selected_import ) {
...
function kp_ocdi_before_widgets_import() {
