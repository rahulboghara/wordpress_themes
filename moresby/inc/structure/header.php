...
if ( ! function_exists( 'zs_enqueue_styles' ) ) {
	...
	function zs_enequeue_styles() {
		...
		if ( apply_filters( 'zs_plan_use_predefined_colors', true ) ) {
			$plan_color_css_file = apply_filters( 'zs_plan_primary_color', 'plan_default_theme_color' );
			wp_enqueue_style( 'plan-section-color', get_template_directory_uri() . '/css/' . $plan_color_css_file . '.css', '', $zs_version );
		}

		if ( apply_filters( 'zs_service_use_predefined_colors', true ) ) {
			$service_color_css_file = apply_filters( 'zs_service_primary_color', 'service_default_theme_color' );
			wp_enqueue_style( 'service-section-color', get_template_directory_uri() . '/css/' . $service_color_css_file . '.css', '', $zs_version );
		}
