...
function lms_ocdi_import_files() {
    $dd_path = trailingslashit( get_template_directory() ) . 'demo-data/';
    return apply_filters( 'lms_ocdi_files_args', array(
        array(
            'import_file_name'             => 'lms',
            'categories'                   => array( 'lms' ),
            ...
            'import_notice'                => esc_html__( 'Import process may take 10-15 minutes. If you facing any issues please contact our support.', 'lms' ),
        ),
    ) );
}

function lms_ocdi_after_import_setup( $selected_import ) {
...
function lms_ocdi_before_widgets_import() {
